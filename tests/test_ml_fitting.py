...
class TestCreateTransformedFeatures:
    ...
    def test_returns_tuple_dataframe_and_series_types(self, sample_features_df: pd.DataFrame) -> None:
    ...
    def test_adds_derived_column(self, sample_features_df: pd.DataFrame, col: str) -> None:
        data, _y = create_transformed_features(sample_features_df)
        assert col in data.columns

    def test_target_is_rating_series(self, sample_features_df: pd.DataFrame) -> None:
        _data, y = create_transformed_features(sample_features_df)
...
class TestSplitTrainTest:
    ...
    def test_respects_test_size(self, sample_features_df: pd.DataFrame, test_size: float) -> None:
        data, _ = create_transformed_features(sample_features_df)
...
class TestFitPipeline:
    ...
    def test_returns_pipeline_type(self, sample_features_df: pd.DataFrame, tmp_path: Path) -> None:
        data, _y = create_transformed_features(sample_features_df)
    ...
    def test_saves_to_disk(self, sample_features_df: pd.DataFrame, tmp_path: Path) -> None:
        data, _y = create_transformed_features(sample_features_df)
        X_train, y_train, _, _ = split_train_test(data)
        save_path = str(tmp_path / "model.joblib")
        ...
        args, _kwargs = mock_dump.call_args
        assert args[1] == save_path

    def test_pipeline_has_preprocess_and_model_steps(self, sample_features_df: pd.DataFrame, tmp_path: Path) -> None:
        data, _ = create_transformed_features(sample_features_df)
        X_train, y_train, _, _ = split_train_test(data)
        with patch("film_recoomendations.ml_fitting.joblib.dump"):
...
class TestMakePredictions:
    ...
    @pytest.fixture
    def fitted_pipe_and_test(self, sample_features_df: pd.DataFrame, tmp_path: Path) -> tuple:
        data, _ = create_transformed_features(sample_features_df)
        X_train, y_train, X_test, y_test = split_train_test(data)
        with patch("film_recoomendations.ml_fitting.joblib.dump"):
            pipe = fit_pipeline(X_train, y_train, save_path=str(tmp_path / "m.joblib"))
    ...
    def test_returns_tuple_dataframe_and_ndarray(self, fitted_pipe_and_test: tuple) -> None:
    ...
    def test_eval_df_has_column(self, fitted_pipe_and_test: tuple, col: str) -> None:
        pipe, X_test, y_test = fitted_pipe_and_test
        eval_df, _ = make_predictions(pipe, X_test, y_test)
        assert col in eval_df.columns

    def test_predictions_clipped_to_half_five(self, fitted_pipe_and_test: tuple) -> None:
        pipe, X_test, y_test = fitted_pipe_and_test
        _, pred_clipped = make_predictions(pipe, X_test, y_test)
        assert np.all(pred_clipped >= 0.5)
        assert np.all(pred_clipped <= 5.0)

    def test_err_equals_pred_minus_rating(self, fitted_pipe_and_test: tuple) -> None:
        pipe, X_test, y_test = fitted_pipe_and_test
        eval_df, _ = make_predictions(pipe, X_test, y_test)
